...
export default function EmotionPage() {
  ...
  useEffect(() => {
    const savedResponses = localStorage.getItem(`pretest_panas_${sessionId}`);
    if (savedResponses) {
      try {
        const parsed = JSON.parse(savedResponses);
        setResponses(parsed);
        console.log('[pre-test/emotion] Restored responses from localStorage');
      } catch (error) {
        console.error('[pre-test/emotion] Failed to parse saved responses:', error);
      }
    }
  }, [sessionId]);
