...
export default function PostTestAnxietyPage() {
  ...
  const handleNext = async () => {
    ...
    try {
      ...
      const scsData = localStorage.getItem(`posttest_scs_${sessionId}`);
      const panasData = localStorage.getItem(`posttest_panas_${sessionId}`);
      ...
      // API로 전송
      const response = await fetch('/api/save-post-test', {
        method: 'POST',
        headers: {
      ...
      if (!response.ok || !data.success) {
        setError(data.error || '사후 검사 결과 저장에 실패했습니다.');
        setIsSaving(false);
        return;
      }

      // 저장 성공 - localStorage 정리
      localStorage.removeItem(`posttest_scs_${sessionId}`);
      localStorage.removeItem(`posttest_panas_${sessionId}`);

      console.log('Post-test completed and saved:', data);

      // 다음 페이지로 이동 (서술형 질문)
      router.push(`/survey/${sessionId}/post-test/qualitative`);
    } catch (error) {
      console.error('Post-test save error:', error);
          ...
          <div className="flex gap-3">
            ...
            <div>
              ...
              <p className="text-base text-muted-foreground leading-relaxed">
                <strong>지금 현재</strong> 다음과 같은 경험을<br />
                얼마나 느끼시는지 선택해주세요
              </p>
            </div>
          </div>
