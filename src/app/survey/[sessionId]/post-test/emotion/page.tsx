...
export default function PostTestEmotionPage() {
  ...
  useEffect(() => {
    const savedResponses = localStorage.getItem(`posttest_panas_${sessionId}`);
    if (savedResponses) {
      try {
        const parsed = JSON.parse(savedResponses);
        setResponses(parsed);
        console.log('[post-test/emotion] Restored responses from localStorage');
      } catch (error) {
        console.error('[post-test/emotion] Failed to parse saved responses:', error);
      }
    }
  }, [sessionId]);
  ...
  const handleNext = () => {
    if (isFormComplete) {
      ...
      localStorage.setItem(`posttest_panas_${sessionId}`, JSON.stringify(responses));
      console.log('Post-test PANAS-SF-10 data saved to localStorage');
      router.push(`/survey/${sessionId}/post-test/anxiety`);
    }
  };
